...
import de.thingweb.repository.recipe.RecipeCollectionHandler;
import de.thingweb.repository.recipe.RecipeHandler;
...
public class Repository {
    ...
    private static List<String> listRecipes() {
        List<String> recipes = new ArrayList<>();

        for (String uri : ThingDescriptionUtils.listRecipes("?s ?p ?o")) {
        	recipes.add(uri.substring(uri.lastIndexOf("/") + 1));
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (RESTServerInstance i : servers) {
            ...
            System.out.println("Add Recipes resource");
            i.add("/recipe", new RecipeCollectionHandler(servers));
            ...
            for (String recipe : listRecipes()) {
                i.add("/recipe/" + recipe, new RecipeHandler(recipe, servers));
            }            
