...
public class ThingDescriptionUtils
{
  ...
  public static List<String> listThingDescriptions(String query) {
	...
	public static List<String> listRecipes(String query) {
		...
		try {
			...
			try (QueryExecution qexec = QueryExecutionFactory.create(q, dataset)) {
				ResultSet result = qexec.execSelect();
				while (result.hasNext()) {
					QuerySolution recipe = result.next();
					if ((recipe.get("g").toString()).contains("recipe")) {
						recipes.add(recipe.get("g").asResource().getURI());
					}
				}
			} catch (Exception e) {
				throw e;
